...
export default function CustomizedHook({ autogames, selectGames }) {
  const {
    getRootProps,
    getInputLabelProps,
    getInputProps,
    ...
    <NoSsr>
      <div>
        <div {...getRootProps()}>
          ...
          <InputWrapper ref={setAnchorEl} className={focused ? 'focused' : ''} id="games" name="selectedGamesId" onChange={(event) => selectGames(event.target.value, 'selectedGamesId')}>
